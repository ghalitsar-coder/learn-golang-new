...
func main() {
	...
	fmt.Println("╚════════════════════════════════════════════════════════════╝")
	fmt.Println("\n1. PIPELINE:")
	fmt.Println("   - Untuk proses SEQUENTIAL yang harus berurutan")
	fmt.Println("   - Setiap stage memproses 1 item, pass ke stage berikutnya")
	fmt.Println("   - Contoh: Validate → Pack → Ship")
	
	fmt.Println("\n2. WORKER POOL:")
	fmt.Println("   - Untuk MEMBATASI jumlah concurrent workers")
	fmt.Println("   - Banyak task independent, tapi resource terbatas")
	fmt.Println("   - Contoh: Max 3 payment processors untuk hindari overload")
	
	fmt.Println("\n3. FAN-OUT / FAN-IN:")
	fmt.Println("   - Untuk DISTRIBUTE work ke multiple workers")
	fmt.Println("   - Lalu MERGE semua hasil jadi satu")
	fmt.Println("   - Contoh: Search di 3 database parallel, merge hasilnya")
	
	fmt.Println("\n📌 Kapan pakai yang mana?")
	fmt.Println("   Pipeline     → Butuh urutan/sequence")
	fmt.Println("   Worker Pool  → Butuh batasan concurrency")
	fmt.Println("   Fan-Out/In   → Butuh speed dengan parallel processing")
	fmt.Println()
}